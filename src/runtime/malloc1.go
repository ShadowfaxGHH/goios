...
func mallocinit() {
	...
	if ptrSize == 8 && (limit == 0 || limit > 1<<30) {
		...
		// 
		// However, on arm64, where the user address space is limited to 39 bits
		// when using 4kb pages, we ignore all this advice above and slam the
		// allocation at 0x40 << 32.
